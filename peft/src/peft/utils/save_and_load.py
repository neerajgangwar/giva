...
def get_peft_model_state_dict(
    ...
    elif config.peft_type == PeftType.VEGA:
        vega_prefix = PEFT_TYPE_TO_PREFIX_MAPPING[config.peft_type]
        to_return = {k: state_dict[k] for k in state_dict if vega_prefix in k or 'vega_A' in k or 'vega_B' in k}
    elif config.peft_type == PeftType.GIVA:
        giva_prefix = PEFT_TYPE_TO_PREFIX_MAPPING[config.peft_type]
        to_return = {k: state_dict[k] for k in state_dict if giva_prefix in k or 'giva_A' in k or 'giva_B' in k}
...
def set_peft_model_state_dict(
    ...
    if config.is_prompt_learning or config.peft_type == PeftType.ADAPTION_PROMPT:
        ...
        peft_model_state_dict = _insert_adapter_name_into_state_dict(
            state_dict, adapter_name=adapter_name, parameter_prefix='vega' if config.peft_type == PeftType.VEGA else parameter_prefix
        )
