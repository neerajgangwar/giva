...
class OsoraModel(BaseTuner):
    """
    Creates Vector-based Random Matrix Adaptation (Osora) model from a pretrained transformers model.
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: OsoraConfig) -> None:
        ...
        for existing_config in self.peft_config.values():
            ...
            if existing_config.projection_prng_key != config.projection_prng_key:
                raise ValueError(
                    f"Osora PRNG initialisation key must be the same for all adapters. Got {config.projection_prng_key=} but "
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        kwargs = {
            ...
            "init_weights": osora_config.init_weights,
    ...
    def _create_new_module(osora_config, adapter_name, target, **kwargs):
        ...
        if loaded_in_8bit:
            raise NotImplementedError(f'8-bit implementation not present')
        elif loaded_in_4bit:
            raise NotImplementedError(f'4-bit implementation not present')
    ...
    def _prepare_adapter_config(peft_config, model_config):
        if peft_config.target_modules is None:
            raise ValueError(f'target_modules is not provided')
        return peft_config
    ...
    def _unload_and_optionally_merge(
        ...
        # we cannot use self.prefix as we want to include non-trainable osora parameters
        key_list = [key for key, _ in self.model.named_modules() if "osora" not in key]
