...
class GivaLayer(BaseTunerLayer):
    # List all names of layers that may contain adapter weights
    adapter_layer_names = ("giva_lambda_o", "giva_lambda_s")
    other_param_names = ("giva_A", "giva_B")

    def __init__(self, base_layer: nn.Module, **kwargs):
        self.base_layer = base_layer
        self.r = {}
        self.giva_dropout = nn.ModuleDict({})

        # For storing vector scale
        self.giva_lambda_o = nn.ParameterDict({})
        self.giva_lambda_s = nn.ParameterDict({})

        # Stores a reference to the vera_A/B BufferDict.
        # Set to `None` otherwise to avoid computation with random weights
        self.giva_A = BufferDict({}, persistent=True)
        self.giva_B = BufferDict({}, persistent=True)
    ...
    def update_layer(
        ...
        if giva_dropout > 0.0:
            giva_dropout_layer = nn.Dropout(p=giva_dropout)
        else:
            giva_dropout_layer = nn.Identity()

        self.giva_dropout.update(nn.ModuleDict({adapter_name: giva_dropout_layer}))

        self.giva_lambda_s[adapter_name] = nn.Parameter(torch.ones(r), requires_grad=True)
        self.giva_lambda_o[adapter_name] = nn.Parameter(torch.ones(self.out_features), requires_grad=True)

        # Non-trainable
        self.giva_A[adapter_name] = torch.randn(r, self.in_features)
        self.giva_B[adapter_name] = torch.randn(self.out_features, r)
    ...
    def init_A_B(self, adapter_name: str, init_weights: str, s_initial: float, o_initial: float, fan_in_fan_out: bool) -> None:
        ...
        self.giva_A[adapter_name].data = A.contiguous()
        self.giva_B[adapter_name].data = B.contiguous()

        if adapter_name in self.giva_lambda_s.keys():
            assert o_initial == 0, f'o_initial must be 0 but got "{o_initial}"'
            assert s_initial == 1, f's_initial must be 1 but got "{s_initial}'
            nn.init.zeros_(self.giva_lambda_s[adapter_name]).fill_(s_initial)
            nn.init.zeros_(self.giva_lambda_o[adapter_name]).fill_(o_initial)


class Linear(nn.Linear, GivaLayer):
    # Giva implemented in a dense layer
    def __init__(
        self,
        base_layer,
        adapter_name: str,
        r: int = 0,
        giva_dropout: float = 0.0,
        ...
        **kwargs,
        ...
        GivaLayer.__init__(self, base_layer, **kwargs)
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.giva_lambda_o.keys():
                base_layer = self.get_base_layer()
                if safe_merge:
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        """
        giva_A = self.giva_A[adapter]
        giva_B = self.giva_B[adapter]

        device = giva_B.device
        dtype = giva_B.dtype
        ...
        lambda_o = self.giva_lambda_o[adapter]
        lambda_s = self.giva_lambda_s[adapter]
        ...
        output_tensor = transpose((lambda_o * giva_B) @ (lambda_s * giva_A), self.fan_in_fan_out)

        if cast_to_fp32:
            output_tensor = output_tensor.to(dtype=dtype)

            self.giva_A[adapter].weight.data = giva_A.to(dtype)
            self.giva_B[adapter].weight.data = giva_B.to(dtype)
            self.giva_lambda_s[adapter].weight.data = lambda_s.to(dtype)
            self.giva_lambda_o[adapter].weight.data = lambda_o.to(dtype)
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            giva_A_keys = self.giva_A.keys()
            for active_adapter in self.active_adapters:
                if active_adapter not in giva_A_keys:
                    continue

                giva_A = self.giva_A[active_adapter]
                giva_B = self.giva_B[active_adapter]
                dropout = self.giva_dropout[active_adapter]
                lambda_s = self.giva_lambda_s[active_adapter]
                lambda_o = self.giva_lambda_o[active_adapter]
                x = self._cast_input_dtype(x, giva_A.dtype)
                result = result + lambda_o * F.linear(lambda_s * F.linear(dropout(x), giva_A), giva_B)
...
class ConcatLinear(Linear):
    ...
    def update_layer(
        ...
        self.giva_lambda_s[adapter_name] = nn.Parameter(torch.ones(r * sum(concat_enabled)), requires_grad=True)
        self.giva_lambda_o[adapter_name] = nn.Parameter(torch.ones(self.out_features * sum(concat_enabled) // len(concat_enabled)), requires_grad=True)

        # Non-trainable
        self.giva_A[adapter_name] = torch.randn(r * sum(concat_enabled), self.in_features)
        self.giva_B[adapter_name] = torch.randn(self.out_features * sum(concat_enabled) // len(concat_enabled), r)
    ...
    def init_A_B(self, adapter_name: str, init_weights: str, s_initial: float, o_initial: float, fan_in_fan_out: bool, concat_enabled: List[bool]) -> None:
        ...
        assert self.giva_A[adapter_name].data.size() == A.size()
        assert self.giva_B[adapter_name].data.size() == B.size()
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        lambda_s = lambda_s.unsqueeze(-1)
        lambda_o = lambda_o.unsqueeze(-1)
        weight_A = lambda_s * giva_A
        weight_B = lambda_o * giva_B
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                giva_output = F.linear(dropout(x), delta_weight)
                result = result + giva_output
