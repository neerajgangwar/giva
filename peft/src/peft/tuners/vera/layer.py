...
class ConcatLinear(Linear):
    ...
    def update_layer(
        ...
        # Actual trainable parameters
        self.vera_lambda_b[adapter_name] = nn.Parameter(torch.ones(self.out_features * sum(concat_enabled) // len(concat_enabled)), requires_grad=True)
        self.vera_lambda_d[adapter_name] = nn.Parameter(torch.randn(r * sum(concat_enabled)), requires_grad=True)
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        sliced_A = vera_A[:, : self.in_features].to(lambda_d.device)
        sliced_A = sliced_A.repeat(sum(concat_enabled), 1)
        sliced_B = vera_B[: self.out_features // len(concat_enabled), :].to(lambda_d.device)
        sliced_B = sliced_B.repeat(sum(concat_enabled), 1)
        lambda_b = lambda_b.unsqueeze(-1)
        lambda_d = lambda_d.unsqueeze(-1)

        weight_A = lambda_d * sliced_A
        weight_B = lambda_b * sliced_B
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                vera_output = F.linear(dropout(x), delta_weight)
                result = result + vera_output
