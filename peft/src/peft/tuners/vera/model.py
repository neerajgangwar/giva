...
from .layer import Linear, VeraLayer, ConcatLinear
...
def _kaiming_init(
    ...
    tensor = tensor.to(generator.device)
    ...
    with torch.no_grad():
        return tensor.uniform_(-bound, bound, generator=generator).to(tensor_device)
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        concat_enabled_key = get_pattern_key(vera_config.concat_enabled_pattern.keys(), current_key)
        concat_enabled = vera_config.concat_enabled_pattern.get(concat_enabled_key, None)
        ...
        elif isinstance(target, ConcatLinear):
            assert concat_enabled is not None
            target.update_layer(
                adapter_name,
                self.vera_A,
                self.vera_B,
    ...
    def _create_new_module(vera_config, vera_A, vera_B, adapter_name, target, **kwargs):
        ...
        layer_cls = Linear if concat_enabled is None else ConcatLinear
