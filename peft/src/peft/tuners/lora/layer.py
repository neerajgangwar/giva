...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        elif isinstance(init_lora_weights, str) and init_lora_weights.lower() in ("lora_ga", "lora_one"):
            with gather_params_ctx(self.get_base_layer().weight):
                self.grad_based_init(adapter_name, init_lora_weights.lower())
    ...
    def grad_based_init(self, adapter_name: str, init_lora_weights: str):
        assert not self.use_dora[adapter_name]
        ...
        if init_lora_weights == "lora_one":
            U, S, V = torch.svd_lowrank(-grad.to(torch.float32).to(device), q=min(16 * r, min(weight.size())), niter=16)
        else:
            U, S, V = torch.svd_lowrank(grad.to(torch.float32).to(device), q=min(16 * r, min(weight.size())), niter=16)
        V = V.T

        if init_lora_weights == "lora_one":
            B = U[:, :r] @ torch.diag(torch.sqrt(S[:r])) / torch.sqrt(S[0])
            A = torch.diag(torch.sqrt(S[:r])) @ V[:r, :] / torch.sqrt(S[0])
        elif init_lora_weights == "lora_ga":
            B = U[:, r : 2 * r]
            A = V[:r, :]
        else:
            NotImplementedError(f'For {init_lora_weights}')
        ...
        if scale == "gd":
          ...
          if init_lora_weights == "lora_one":
            gamma = self.kwargs["stable_gamma"]
            B = B / gamma**0.5
            A = A / gamma**0.5
          else:
            ...
            _, S, _ = torch.svd_lowrank(self.get_base_layer().weight.float(), q=4 * r, niter=4)
            S = S / scaling_factor
            avg_s = torch.sqrt(S[:r]).mean().to(A.device)
            B = B * avg_s
            A = A * avg_s
        else:
            raise NotImplementedError(f'For {scale}')
        ...
        with torch.no_grad():
            if init_lora_weights != 'lora_one':
                offset = scaling_factor * B @ A
                self.get_base_layer().weight.data = weight.data - offset
...
class ConcatLinear(Linear):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert not use_dora, 'DoRA is not supported'
        assert not lora_bias, 'lora_bias is not supported'
        assert isinstance(init_lora_weights, bool), 'Only bool values for init_lora_weights are supported'
    ...
    def update_layer(
        ...
        assert not lora_bias, f'lora_bias must be False for using ConcatLinear'
        ...
        self.lora_dropout.update(nn.ModuleDict({adapter_name: lora_dropout_layer}))
        # Actual trainable parameters
        self.lora_A[adapter_name] = nn.Linear(self.in_features, r * sum(concat_enabled), bias=False)
        self.lora_B[adapter_name] = nn.Linear(r, self.out_features * sum(concat_enabled) // len(concat_enabled), bias=lora_bias)
        self.lora_bias[adapter_name] = lora_bias
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                x = self._cast_input_dtype(x, lora_A.weight.dtype)
                if active_adapter not in self.lora_variant:  # vanilla LoRA
                    delta_weight = transpose(self.get_delta_weight(active_adapter), self.fan_in_fan_out) # out_features x in_features
                    lora_output = F.linear(dropout(x), delta_weight)
...
def dispatch_default(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        kwargs.update(lora_config.loftq_config)
        if concat_enabled:
            new_module = ConcatLinear(target, adapter_name, is_target_conv_1d_layer=True, **kwargs)
        else:
