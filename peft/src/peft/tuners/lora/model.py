...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        ...
        if hasattr(self, "named_grad"):
            delattr(self, "named_grad")
    ...
    def _create_and_replace(
        ...
        concat_enabled_key = get_pattern_key(lora_config.concat_enabled_pattern.keys(), current_key)
        r = lora_config.rank_pattern.get(r_key, lora_config.r)
        alpha = lora_config.alpha_pattern.get(alpha_key, lora_config.lora_alpha)
        concat_enabled = lora_config.concat_enabled_pattern.get(concat_enabled_key, None)
        ...
        if lora_config.init_lora_weights in ('lora_ga', 'lora_one') and self.named_grad is not None:
            kwargs.update({
                'grad': self.named_grad[current_key],
                'scale': lora_config.scale,
                'stable_gamma': lora_config.stable_gamma,
            })
        if concat_enabled is not None:
            concat_enabled_args = {
                'concat_enabled': concat_enabled,
            }
            kwargs.update(concat_enabled_args)
        else:
            concat_enabled_args = {}
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        for dispatcher in dispatchers:
            ...
            if new_module is not None:  # first match wins
                assert concat_enabled is None or dispatcher is dispatch_default
                break
